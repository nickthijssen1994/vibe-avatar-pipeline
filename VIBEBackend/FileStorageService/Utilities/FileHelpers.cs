...
namespace FileStorageService.Utilities
{
	public static class FileHelpers
	{
		public static readonly string[] PermittedExtensions = { ".jpg", ".jpeg", ".png", ".mp4", ".mkv" };

		// If you require a check on specific characters in the IsValidFileExtensionAndSignature
		// method, supply the characters in the _allowedChars field.
		private static readonly byte[] _allowedChars = { };

		// For more file signatures, see (https://en.wikipedia.org/wiki/List_of_file_signatures)
		...
		private static readonly Dictionary<string, List<byte[]>> _fileSignature = new()
		{
			...
			},
			{
				".mp4", new List<byte[]>
				{
					...
					new byte[] { 0x0, 0x0, 0x0, 0x20, 0x66, 0x74, 0x79, 0x70 },
				}
			},
			{
				".mkv", new List<byte[]>
		...
		public static async Task<byte[]> ProcessStreamedFile(MultipartSection section,
			ContentDispositionHeaderValue contentDisposition, ModelStateDictionary modelState,
			string[] permittedExtensions, long sizeLimit)
		{
			try
			{
